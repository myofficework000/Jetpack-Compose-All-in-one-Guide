...
class NewsAPIResitoryMockitoTest {
   ...
   @Test
   fun `Mockito, Given news data is empty, When fetching news data, Then returns empty list `() = runTest {
      Mockito.`when`(apiService.getTopHeadlines(Constants.NEWS_API_COUNTRY, Constants.NEWS_API_KEY))
         .thenReturn(
            Response.success(
   ...
   @Test
   fun `Mockito, Given news data is NOT empty, When fetching news data, Then returns NOT empty list `() = runTest {
      Mockito.`when`(apiService.getTopHeadlines(Constants.NEWS_API_COUNTRY, Constants.NEWS_API_KEY))
         .thenReturn(
            Response.success(
   ...
   @Test
   fun `Mockito, Given news data is empty, When fetching news data, Then returns error `() = runTest {
