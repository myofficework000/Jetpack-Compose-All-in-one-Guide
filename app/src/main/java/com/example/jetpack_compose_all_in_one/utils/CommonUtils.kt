    ...
    if (pages.size > 1) {
        Box(
            ...
            Row(
                ...
                SimpleTextButton("Prev", enabled = currentPage.value > 0) {
                ...
                SimpleTextButton("Next", enabled = currentPage.value < pages.size - 1) {
                    if (currentPage.value < cleanupPerPage.size) cleanupPerPage[currentPage.value]()
                    cleanupAllPages()
                    currentPage.value++
                    pages[currentPage.value]()
...
private fun LoginPagerPreview() {
    ...
    LogicPager(
        ...
        Box(Modifier.fillMaxSize().padding(it).background(Color.Yellow)) {}
    }
}